...
<div class="w-8 h-8 mx-auto flex items-center justify-center">
<img src="https://readdy.ai/api/search-image?query=icon%2C%203D%20cartoon%2C%20sad%20emoji%20face%2C%20the%20icon%20should%20take%20up%2070%25%20of%20the%20frame%2C%20soft%20blue%20color%20with%20gentle%20gradients%2C%20minimalist%20design%2C%20smooth%20rounded%20shapes%2C%20subtle%20shading%2C%20no%20outlines%2C%20centered%20composition%2C%20isolated%20on%20white%20background%2C%20emotional%20and%20sympathetic%20aesthetic%2C%20high%20detail%20quality%2C%20clean%20and%20modern%20look&width=40&height=40&seq=8&orientation=squarish" alt="Sedih" class="w-full h-full object-contain">
</div>
...
<button id="nav-home" class="flex flex-col items-center justify-center w-1/5 text-primary cursor-pointer">
<div class="w-6 h-6 flex items-center justify-center">
<i class="ri-home-fill ri-lg"></i>
</div>
<span class="text-xs mt-1">Home</span>
</button>
<button id="nav-chat" class="flex flex-col items-center justify-center w-1/5 text-gray-500 cursor-pointer">
<div class="w-6 h-6 flex items-center justify-center">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.querySelectorAll('.mood-card').forEach(card => {
    card.addEventListener('click', function() {
      document.querySelectorAll('.mood-card').forEach(c => c.classList.remove('ring', 'ring-primary', 'scale-105'));
  ...
  function respondToMessage(userMessage) {
    let response = "Terima kasih sudah berbagi. Aku mengerti perasaanmu. Bagaimana menurutmu jika kita mencoba beberapa teknik pernapasan atau aktivitas yang bisa membantu menenangkan pikiranmu?";
    if (userMessage.toLowerCase().includes('sedih')) {
      response = "Aku mengerti kamu sedang merasa sedih. Tidak apa-apa untuk merasakan emosi ini. Maukah kamu bercerita lebih lanjut tentang apa yang membuatmu sedih? Aku di sini untuk mendengarkan.";
    } else if (userMessage.toLowerCase().includes('cemas') || userMessage.toLowerCase().includes('khawatir')) {
      response = "Kecemasan bisa sangat tidak nyaman. Coba tarik napas dalam-dalam beberapa kali. Inhale... exhale... Apa yang membuatmu merasa cemas saat ini?";
    } else if (userMessage.toLowerCase().includes('marah')) {
      response = "Aku mengerti kamu sedang merasa marah. Emosi ini wajar dan valid. Mungkin kita bisa mencari cara untuk menyalurkan energi ini dengan cara yang sehat. Apa yang biasanya membantumu saat merasa marah?";
    } else if (userMessage.toLowerCase().includes('senang') || userMessage.toLowerCase().includes('bahagia')) {
      response = "Senang sekali mendengar kamu merasa bahagia! Momen-momen positif seperti ini penting untuk diapresiasi. Apa yang membuatmu merasa senang hari ini?";
    } else if (userMessage.toLowerCase().includes('butuh bantuan') || userMessage.toLowerCase().includes('tolong')) {
      response = "Aku ingin kamu tetap tenang dan perlahan tarik nafas dalam, kami disini akan membantumu melalui tombol bantuan darurat di sebelah kanan atas";
    }
